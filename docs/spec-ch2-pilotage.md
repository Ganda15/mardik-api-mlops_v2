...
> **Statut : VALIDÉE le 23/09/2026.** Réponses aux questions du §8 : échéance du 24/09 confirmée ; rollback
...
| Gate réel et livraison (réserve A2, `modele-ci`) | idée de David : tag `eval-ok/<sha>` | **brique 21** : `gate-reel.yml` (manuel, Azure, secrets du dépôt, rapport en artefact, tag `eval-ok/<sha>`) ; `llmops.yml` refuse de publier sans ce tag **si** la variable de dépôt `MARDIK_EXIGER_EVAL_OK=1`, et publie alors depuis le vrai rapport | la variable évite de bloquer `main` avant que les secrets existent — secrets et variable posés le 23/09 ; le workflow a tourné sur Azure le soir même (des passages refusés sur la latence, les autres passés) |
...
| **21** ✅ | gate réel manuel `gate-reel.yml`, tag `eval-ok/<sha>`, publication qui l'exige sur variable — **exécuté sur Azure depuis le 23/09**, des passages refusés sur la latence (P95 12,5 s ; 8,7 s) | `tests/test_gate_reel.py` |
